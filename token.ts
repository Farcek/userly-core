...
export class ConsoleToken {
    private static pass = 'asdfv234234af';
    static gen(userid: string, usertoken: string): string {
        return ICommon.jwtEncryption({ u: userid, t: usertoken }, { pass: ConsoleToken.pass, jwtid: 'userlyconsole', expiresIn: '1h' });
    }
    static parse(token: string): IConsoleUsertoken {
        var data = ICommon.jwtDecryption<{ u: string, t: string }>(token, { pass: ConsoleToken.pass, jwtid: 'userlyconsole' });
        //console.log('data', data)
        return {
            userid: data.u, usertoken: data.t
        }
    }
}